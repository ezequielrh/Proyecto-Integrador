...
void main_thread_create(void)
{
    ...
    UINT err_g_sw5_semaphore;
    err_g_sw5_semaphore = tx_semaphore_create (&g_sw5_semaphore, (CHAR *) "SW5 Semaphore", 0);
    if (TX_SUCCESS != err_g_sw5_semaphore)
    {
        tx_startup_err_callback (&g_sw5_semaphore, 0);
    }
    UINT err_g_adc_semaphore;
    err_g_adc_semaphore = tx_semaphore_create (&g_adc_semaphore, (CHAR *) "ADC Semaphore", 0);
    if (TX_SUCCESS != err_g_adc_semaphore)
    {
        tx_startup_err_callback (&g_adc_semaphore, 0);
    }
