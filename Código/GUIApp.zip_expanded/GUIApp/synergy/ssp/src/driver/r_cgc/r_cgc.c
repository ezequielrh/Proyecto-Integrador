...
ssp_err_t R_CGC_ClocksCfg(cgc_clocks_cfg_t const * const p_clock_cfg)
{
    ...
    /* With PLL clock, we cannot go to sub-osc/low-speed, so we don't have to check if we have to rerun
     * r_cgc_operating_mode_set().  */
    if (CGC_CLOCK_PLL != requested_system_clock)
    {
        /* Check if clocks PLL, MOSC, HOCO and MOCO are stopped as a precondition to enter Subosc Speed Mode */
        if (false == r_cgc_subosc_mode_possible())
        {
            /* At least one clock not stopped, set a reminder to retry */
            power_mode_adjustment = true;
        }
    }
...
ssp_err_t R_CGC_ClockStart (cgc_clock_t clock_source, cgc_clock_cfg_t * p_clock_cfg)
{
    ...
    else
    {
        ...
        if (CGC_SUBOSC_SPEED_MODE == HW_CGC_OperatingModeGet(gp_system_reg))
        {
            if ((CGC_CLOCK_HOCO == clock_source) || (CGC_CLOCK_MOCO == clock_source)|| (CGC_CLOCK_MAIN_OSC == clock_source))
            {
...
ssp_err_t R_CGC_SystemClockSet (cgc_clock_t clock_source, cgc_system_clock_cfg_t const * const p_clock_cfg)
{
    ...
    if (clock_source != current_clock)    // if clock_source is not the current system clock, check stabilization
    {
        ...
        else if ((CGC_CLOCK_MOCO == clock_source) || (CGC_CLOCK_LOCO == clock_source))
        {
...
ssp_err_t R_CGC_SystemClockFreqGet (cgc_system_clocks_t clock, uint32_t * p_freq_hz)
{
    ...
    {
        CGC_ERROR_RETURN(gp_cgc_feature->has_fclk, SSP_ERR_INVALID_ARGUMENT);
    }
    else if (CGC_SYSTEM_CLOCKS_BCLK == clock)
    {
        CGC_ERROR_RETURN(gp_cgc_feature->has_bclk, SSP_ERR_INVALID_ARGUMENT);
    }
    else
    {
 ...
 * @return SSP_ERR_INVALID_MODE         Invalid peripheral clock divider setting. Frequencies of peripherals should
...
ssp_err_t R_CGC_LCDClockCfg (cgc_clock_t clock)
{
    ...
    /* check if the MCU has lcd_clock */
    CGC_ERROR_RETURN((1U == gp_cgc_feature->has_lcd_clock),SSP_ERR_UNSUPPORTED);
...
{
    /* check if the MCU has sdadc_clock */
    CGC_ERROR_RETURN((1U == gp_cgc_feature->has_sdadc_clock),SSP_ERR_UNSUPPORTED);
...
{
    /* check if the MCU has sdram_clock */
    CGC_ERROR_RETURN((1U == gp_cgc_feature->has_sdram_clock),SSP_ERR_UNSUPPORTED);
...
ssp_err_t R_CGC_USBClockCfg (cgc_usb_clock_div_t divider)
{
    ...
    /* checks if the MCU has usb_clock divisor */
    CGC_ERROR_RETURN((1U == gp_cgc_feature->has_usb_clock_div),SSP_ERR_INVALID_ARGUMENT);
...
static ssp_err_t r_cgc_check_peripheral_clocks (cgc_system_clocks_t clock)
{
    ...
    {
        CGC_ERROR_RETURN(gp_cgc_feature->has_pclka, SSP_ERR_INVALID_ARGUMENT);
    }
    else if (CGC_SYSTEM_CLOCKS_PCLKB == clock)
    {
        CGC_ERROR_RETURN(gp_cgc_feature->has_pclkb, SSP_ERR_INVALID_ARGUMENT);
    }
    else if (CGC_SYSTEM_CLOCKS_PCLKC == clock)
    {
        CGC_ERROR_RETURN(gp_cgc_feature->has_pclkc, SSP_ERR_INVALID_ARGUMENT);
    }
    else if (CGC_SYSTEM_CLOCKS_PCLKD == clock)
    {
        CGC_ERROR_RETURN(gp_cgc_feature->has_pclkd, SSP_ERR_INVALID_ARGUMENT);
    }
    else
    {
        return SSP_ERR_INVALID_ARGUMENT;
...
/*******************************************************************************************************************//**
 * @brief  Verifies if Sub-osc Mode is possible. This mode requires the PLL (if present), MOSC, HOCO and MOCO clocks stopped
 **********************************************************************************************************************/
static bool r_cgc_subosc_mode_possible(void)
{
    if (0U == gp_cgc_feature->pllccr_type)
    {
        if ((false == HW_CGC_ClockRunStateGet(gp_system_reg, CGC_CLOCK_MAIN_OSC)) &&
            (false == HW_CGC_ClockRunStateGet(gp_system_reg, CGC_CLOCK_HOCO)) && (false == HW_CGC_ClockRunStateGet(gp_system_reg, CGC_CLOCK_MOCO)))
        {
            return true;
        }
    }
    ...
    {
        if ((false == HW_CGC_ClockRunStateGet(gp_system_reg, CGC_CLOCK_PLL)) && (false == HW_CGC_ClockRunStateGet(gp_system_reg, CGC_CLOCK_MAIN_OSC)) &&
            (false == HW_CGC_ClockRunStateGet(gp_system_reg, CGC_CLOCK_HOCO)) && (false == HW_CGC_ClockRunStateGet(gp_system_reg, CGC_CLOCK_MOCO)))
        {
            return true;
        }
...
/*******************************************************************************************************************//**
 * @brief  Verifies if Low-speed Mode is possible. This mode requires the PLL (if present) clock stopped
 **********************************************************************************************************************/
static bool r_cgc_low_speed_mode_possible(void)
{
    if ((0U == gp_cgc_feature->pllccr_type) || (false == HW_CGC_ClockRunStateGet(gp_system_reg, CGC_CLOCK_PLL)))
    {
        return true;
    }
    return false;
...
static void r_cgc_operating_mode_set(cgc_clock_t const clock_source, uint32_t const current_system_clock)
{
    ...
    else
    {
         ...
         * That also implies that source is SCLK or LOCO  
         * Checks if MCU supports sub oscillator speed mode */
        if ((0U != gp_cgc_feature->has_subosc_speed) && (true == r_cgc_subosc_mode_possible()))
        {
        ...
        else if ((current_system_clock <= gp_cgc_feature->low_speed_max_freq_hz) && (true == r_cgc_low_speed_mode_possible()))
        {
            /* Switch to low speed mode */
            HW_CGC_SetLowSpeedMode(gp_system_reg);
        }
        /* System clock should be less than or equal to Middle speed max frequency */
        else if (current_system_clock <= gp_cgc_feature->middle_speed_max_freq_hz)
        {
            /* Switch to middle speed mode */
            HW_CGC_SetMiddleSpeedMode(gp_system_reg);
        }
